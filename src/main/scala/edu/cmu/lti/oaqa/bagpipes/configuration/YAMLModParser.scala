...
class YAMLModParser(baseDir: Option[String] = None) extends ModParser(baseDir) {
...
object YAMLModParser {
  def apply(baseDir: Option[String] = None) = new YAMLModParser(baseDir)
}